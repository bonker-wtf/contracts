...
/// @title BonkerLpLockerFeeConversion
/// @notice LP locker that holds Uniswap v4 positions and distributes accrued LP fees to reward recipients.
///         Each recipient has a configurable preference (FeeIn.Bonker or FeeIn.Paired): fees collected in the
///         wrong currency are swapped in a single batch before distribution to minimise swap overhead.
/// @dev Collection is skipped while the MEV module is active to prevent swap-backs from being blocked.
///      The `_inCollect` guard stops recursive collection triggered by the swap-back itself.
contract BonkerLpLockerFeeConversion is IBonkerLpLockerFeeConversion, ReentrancyGuard, Ownable {
    ...
    /// @notice Returns the reward configuration for `token`.
    /// @param token The bonker token address.
    /// @return Full `TokenRewardInfo` struct including pool key, position IDs, and recipient arrays.
    function tokenRewards(address token) external view returns (TokenRewardInfo memory) {
        return _tokenRewards[token];
    }

    /// @notice Called by the factory at token deployment to mint LP positions and register reward recipients.
    /// @dev Only callable by the factory. `lockerConfig.lockerData` must encode `LpFeeConversionInfo`.
    ///      `rewardBps` must sum to BASIS_POINTS, all arrays must have equal length (max MAX_REWARD_PARTICIPANTS),
    ///      and no zero addresses are permitted. Multiple LP positions are split by `positionBps`.
    ///      Minted position NFTs use consecutive IDs starting at `positionId`.
    /// @param lockerConfig Position and reward config; `lockerData` encodes fee conversion preferences.
    /// @param poolConfig Pool parameters (pairedToken, tickSpacing, tickIfToken0IsBonker).
    /// @param poolKey The Uniswap v4 pool key for the token.
    /// @param poolSupply Total token amount to deposit across all LP positions.
    /// @param token The bonker ERC20 token being locked.
    /// @return positionId The first minted position NFT id.
    function placeLiquidity(
        ...
        FeeIn[] memory lpFeeConversionPreferences = abi.decode(
            lockerConfig.lockerData, (IBonkerLpLockerFeeConversion.LpFeeConversionInfo)
        ).feePreference;
    ...
    /// @notice Collects LP fees while the Uniswap v4 pool is already unlocked (e.g. from an afterSwap hook).
    /// @dev Uses `modifyLiquiditiesWithoutUnlock`. Re-entrant calls return early via `_inCollect`.
    ///      No-ops if the MEV module is still active.
    /// @param token The bonker token whose fees are being collected.
    function collectRewardsWithoutUnlock(address token) external {
        _collectRewards(token, true);
    }

    /// @notice Collects LP fees via the standard locked path (opens a new Uniswap v4 lock internally).
    /// @dev Uses the Universal Router for the swap-back. No-ops if the MEV module is still active.
    /// @param token The bonker token whose fees are being collected.
    ...
    function _mevModuleOperating(address token) internal view returns (bool) {
        ...
        if (!IBonkerHook(address(_tokenRewards[token].poolKey.hooks)).mevModuleEnabled(poolId)) {
            return false;
        }
        ...
        uint256 poolCreationTimestamp =
            IBonkerHook(address(_tokenRewards[token].poolKey.hooks)).poolCreationTimestamp(poolId);
        if (
            poolCreationTimestamp
                + IBonkerHook(address(_tokenRewards[token].poolKey.hooks)).MAX_MEV_MODULE_DELAY()
                <= block.timestamp
        ) {
            return false;
        }
        ...
        returns (uint256[] memory, uint256[] memory)
    {
        ...
        // if the rewardToken is the bonker, we want to swap for recipients who
        // want their fee in the paired token
        //
        // conversely, if the rewardToken is the paired token, we want to swap for
        // recipients who want their fee in the bonker token
        FeeIn toSwap = token == rewardToken ? FeeIn.Paired : FeeIn.Bonker;
    ...
    /// @notice Updates the fee recipient for a reward slot.
    /// @dev Only the admin for `rewardIndex` can call this.
    /// @param token The bonker token.
    /// @param rewardIndex Index into the rewards arrays.
    /// @param newRecipient The new address to receive fees for this slot.
    ...
    /// @notice Changes whether a reward slot receives fees in both tokens, the bonker token, or the paired token.
    /// @dev Only the admin for `rewardIndex` can call this. Takes effect on the next fee collection.
    /// @param token The bonker token.
    /// @param rewardIndex Index into the rewards arrays.
    /// @param newFeePreference FeeIn.Both, FeeIn.Bonker, or FeeIn.Paired.
    ...
    /// @notice Transfers admin rights for a reward slot to a new address.
    /// @dev Only the current admin for `rewardIndex` can call this. The new admin controls recipient,
    ///      fee preference, and future admin updates for this slot.
    /// @param token The bonker token.
    /// @param rewardIndex Index into the rewards arrays.
    /// @param newAdmin The new admin address for this slot.
    ...
    /// @notice Accepts LP position NFTs from the factory during `placeLiquidity`.
    /// @dev Rejects transfers from any address other than the factory.
    ...
    /// @notice Emergency withdrawal of ETH stranded in this contract.
    /// @dev Only callable by owner. ETH can accumulate from unexpected direct transfers.
    /// @param recipient Address to receive the ETH.
    function withdrawETH(address recipient) public onlyOwner nonReentrant {
        payable(recipient).transfer(address(this).balance);
    }

    /// @notice Emergency withdrawal of ERC20 tokens stranded in this contract.
    /// @dev Only callable by owner. Transfers the full balance of `token`.
    ...
    /// @notice Returns true for IERC721Receiver and IBonkerLpLocker interface IDs.
