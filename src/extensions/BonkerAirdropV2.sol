...
import {IBonkerAirdropV2} from "./interfaces/IBonkerAirdropV2.sol";
...
contract BonkerAirdropV2 is ReentrancyGuard, IBonkerAirdropV2 {
    ...
    function receiveTokens(
        ...
        if (airdrops[token].totalSupply != 0) {
    ...
    function updateMerkleRoot(address token, bytes32 newMerkleRoot) external {
        ...
        bytes32 oldMerkleRoot = airdrop.merkleRoot;

        // update the merkle root
        airdrop.merkleRoot = newMerkleRoot;

        emit AirdropMerkleRootUpdated(token, oldMerkleRoot, newMerkleRoot);
    }
