...
contract BonkerPresaleAllowlist is IBonkerPresaleAllowlist {
    error InvalidProof();
    ...
    function getAllowedAmountForBuyer(uint256 presaleId, address buyer, bytes calldata proof)
        ...
        returns (uint256)
    {
        PresaleAllowlist storage allowlist = allowlists[presaleId];
        ...
        uint256 addressOverride = allowlist.addressOverrides[buyer];
        if (addressOverride > 0) {
        ...
        if (allowlist.merkleRoot == bytes32(0)) {
