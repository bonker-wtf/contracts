...
import {IBonkerSniperAuctionV0} from "../BonkerSniperAuctionV0.sol";
...
// shared util for snipers to bid in BonkerSniperAuction V0 and V2 auctions
contract BonkerSniperUtilV2 is ReentrancyGuard {
    ...
    IBonkerSniperAuctionV0 immutable bonkerSniperAuction;
    IUniversalRouter immutable universalRouter;
    IPermit2 immutable permit2;
    address immutable weth;

    constructor(
        address _bonkerSniperAuction,
        address _universalRouter,
        address _permit2,
        address _weth
    ) {
        bonkerSniperAuction = IBonkerSniperAuctionV0(_bonkerSniperAuction);
        ...
        returns (uint256 txGasPrice)
    {
        ...
        if (desiredBidAmount % bonkerSniperAuction.paymentPerGasUnit() != 0) {
            revert InvalidBidAmount();
        }

        // calculate the gas price for the bid amount
        txGasPrice = auctionGasPeg + (desiredBidAmount / bonkerSniperAuction.paymentPerGasUnit());
    }
        ...
        nonReentrant
    {
        ...
        if (round != bonkerSniperAuction.round(poolId) || round > bonkerSniperAuction.maxRounds())
        {
            revert InvalidRound();
        }

        // check that the correct block is being bid in
        if (block.number != bonkerSniperAuction.nextAuctionBlock(poolId)) {
            revert InvalidBlock();
        }

        // check that the gas price is high enough
        int256 paymentUnits = int256(tx.gasprice) - int256(bonkerSniperAuction.gasPeg(poolId));
        if (paymentUnits < 0) {
            revert GasPriceTooLow();
        }

        // check proper msg.value was sent in
        uint256 expectedBidAmount = bonkerSniperAuction.paymentPerGasUnit() * uint256(paymentUnits);
        ...
        IERC20(weth).approve(address(bonkerSniperAuction), msg.value);
        ...
        if (
            IBonkerHookV2(address(swapParams.poolKey.hooks)).supportsInterface(
                type(IBonkerHookV2).interfaceId
            )
        ) {
            // v2 hook
            swapParams.hookData = abi.encode(
                IBonkerHookV2.PoolSwapData({
        ...
        if (round + 1 != bonkerSniperAuction.round(poolId)) {
