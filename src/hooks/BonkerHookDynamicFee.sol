...
/// @title BonkerHookDynamicFee
/// @notice Extends {BonkerHook} with a volatility-responsive LP fee model.  The fee rises
///         proportionally to recent tick movement and decays back to a configurable base when
///         the market is calm, helping LPs capture more fees during volatile periods.
contract BonkerHookDynamicFee is BonkerHook, IBonkerHookDynamicFee {
    ...
    /// @notice Returns the static dynamic-fee configuration for a pool.
    ...
    /// @notice Returns the latest dynamic-fee runtime state for a pool.
    /// @param poolId The pool identifier.
    function poolFeeVars(PoolId poolId) external view returns (PoolDynamicFeeVars memory) {
        return _poolFeeVars[poolId];
    }

    /// @notice Decodes and stores the dynamic-fee configuration for a new pool.
    /// @dev Reverts if `baseFee` is below {MIN_BASE_FEE}, `maxLpFee` exceeds {MAX_LP_FEE}, or
    ///      `baseFee` is greater than `maxLpFee`. Emits {PoolInitialized}.
    ...
    /// @notice Computes and applies the LP fee before each swap based on current volatility.
    ...
    /// @notice Converts a volatility accumulator value into an LP fee capped at `maxLpFee`.
    /// @param volAccumulator Tick-distance-based volatility measure for the pool.
    /// @param poolId The pool whose configuration bounds the returned fee.
    /// @return lpFee The clamped LP fee in millionths (e.g. 3000 = 0.3 %).
    ...
    /// @notice Measures the effective tick distance from the reference point, applying decay when
    ///         the filter period has elapsed, and resets the accumulator after a quiet reset period.
    /// @param poolKey Pool being evaluated.
    /// @param swapParams Swap parameters used to simulate the post-swap tick.
    /// @return volatilityAccumulator The updated volatility measure (clamped to uint24).
    ...
    /// @notice Derives the post-swap tick via a self-call simulation that intentionally reverts.
    /// @dev Uses the {TickReturned} revert pattern: calls `simulateSwap`, which always reverts
    ///      with the post-swap tick encoded in the revert data, then decodes it here.
    /// @param poolKey Pool to simulate the swap against.
    /// @param swapParams Swap parameters to simulate.
    /// @return tickAfter Estimated tick after the swap completes.
        ...
        returns (int24 tickAfter)
    {
        ...
        try this.simulateSwap(poolKey, swapParams) {
            ...
            // Decode the int24 value (assuming it's at the end)
            assembly {
                ...
                // For int24, we only need the last 3 bytes of the last word
                // This assumes the int24 is right-aligned in the last word
                let lastWord := mload(lastWordPtr)

                // Sign-extend from the last 3 bytes (24 bits)
                tickAfter := signextend(2, lastWord)
            }
        }
    }

    /// @notice Simulates a swap to derive the post-swap tick for fee calculations.
    /// @param poolKey The pool being simulated.
    /// @param swapParams The swap parameters to simulate.
