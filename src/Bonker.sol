...
import {BonkerDeployer} from "./utils/BonkerDeployer.sol";
...
bytes32 constant BONKER_PROTOCOL_ID = keccak256("bonker.wtf");
/*
╔════════════════════════════════════════════════════════════════╗
║                                                                ║
║      ██████╗  ██████╗ ███╗   ██╗██╗  ██╗███████╗██████╗        ║
...
contract Bonker is OwnerAdmins, ReentrancyGuard, IBonker {
    ...
    /// @notice Toggles whether originating-chain deployments are allowed.
    /// @param deprecated_ True to pause deployments, false to re-enable them.
    function setDeprecated(bool deprecated_) external onlyOwner {
        deprecated = deprecated_;
        emit SetDeprecated(deprecated_);
    }

    /// @notice Sets the address that receives claimed protocol fees.
    /// @param teamFeeRecipient_ The new team fee recipient.
    ...
    /// @notice Transfers the factory's accumulated protocol fees for a token to the team recipient.
    /// @param token The ERC-20 token to claim from the factory balance.
    ...
    /// @notice Returns stored deployment metadata for a token created by the factory.
    ...
    /// @notice Enables or disables a hook contract for new deployments.
    /// @param hook The hook contract address.
    /// @param enabled Whether the hook can be used by the factory.
    function setHook(address hook, bool enabled) external onlyOwnerOrAdmin {
        // check that the hook supports the IBonkerHook interface
        if (!IBonkerHook(hook).supportsInterface(type(IBonkerHook).interfaceId)) {
            revert InvalidHook();
        }
    ...
    /// @notice Enables or disables a locker for a specific hook.
    /// @param locker The locker contract address.
    /// @param hook The hook the locker is compatible with.
    /// @param enabled Whether the locker can be used for that hook.
    function setLocker(address locker, address hook, bool enabled) external onlyOwnerOrAdmin {
        // check that the locker supports the IBonkerLpLocker interface
        if (!IBonkerLpLocker(locker).supportsInterface(type(IBonkerLpLocker).interfaceId)) {
    ...
    /// @notice Enables or disables an MEV module for new deployments.
    /// @param mevModule The MEV module address.
    /// @param enabled Whether the module can be used by the factory.
    function setMevModule(address mevModule, bool enabled) external onlyOwnerOrAdmin {
        // check that the mev module supports the IBonkerMevModule interface
        if (!IBonkerMevModule(mevModule).supportsInterface(type(IBonkerMevModule).interfaceId)) {
    ...
    /// @notice Enables or disables an extension contract for deployments.
    /// @param extension The extension contract address.
    /// @param enabled Whether the extension can be used by the factory.
    function setExtension(address extension, bool enabled) external onlyOwnerOrAdmin {
        // check that the extension contract supports the IBonkerExtension interface
        if (!IBonkerExtension(extension).supportsInterface(type(IBonkerExtension).interfaceId)) {
    ...
    /// @notice Deploys a zero-supply token on a non-originating chain for bridging flows.
    /// @param tokenConfig The token metadata and admin configuration.
        ...
        returns (address tokenAddress)
    {
        ...
        tokenAddress = BonkerDeployer.deployToken(tokenConfig, TOKEN_SUPPLY);
    }

    /// @notice Deploys a token, initializes its pool, and runs enabled extensions and MEV setup.
    /// @param deploymentConfig Full token, pool, locker, extension, and MEV configuration.
    /// @return tokenAddress The deployed token address.
        ...
        returns (address tokenAddress)
    {
        ...
        // deploy the token
        tokenAddress = BonkerDeployer.deployToken(deploymentConfig.tokenConfig, TOKEN_SUPPLY);
        ...
        emit TokenCreated({
            ...
            poolId: poolKey.toId(),
            startingTick: deploymentConfig.poolConfig.tickIfToken0IsBonker,
        ...
        internal
    {
        ...
        // initialize the mev module
        IBonkerHook(deploymentConfig.poolConfig.hook).initializeMevModule(
    ...
    function _initializePool(
        ...
        poolKey = IBonkerHook(poolConfig.hook).initializePool(
            newToken,
            poolConfig.pairedToken,
            poolConfig.tickIfToken0IsBonker,
    ...
    function _initializeLiquidity(
        ...
        IBonkerLpLocker(lockerConfig.locker).placeLiquidity(
    ...
    function _triggerExtensions(
        ...
        for (uint256 i = 0; i < deploymentConfig.extensionConfigs.length; i++) {
            ...
            IBonkerExtension(deploymentConfig.extensionConfigs[i].extension).receiveTokens{
